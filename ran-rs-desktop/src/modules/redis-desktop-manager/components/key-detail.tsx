...
import { Delete, Edit, RefreshRight, Timer, CopyDocument } from '@element-plus/icons-vue';
...
const KeyDetail = defineComponent({
  ...
  setup() {
    ...
    const activeKeyTab = computed(() => {
      ...
      if (tab?.type !== 'key-detail') return null;
      return tab;
    });
    ...
    async function handleRefresh() {
      if (!activeKeyTab.value?.key) return;
    ...
    async function confirmRename() {
      if (!activeKeyTab.value?.key || !renameNewKey.value.trim()) return;
        ...
        <div class={ns.b()}>
          ...
          <el-dialog v-model={renameVisible.value} title="重命名 Key" width="460px" append-to-body>
            ...
            {{
              footer: () => (
                <div>
                  <el-button onClick={() => { renameVisible.value = false; }}>取消</el-button>
          ...
          <el-dialog v-model={ttlVisible.value} title="设置 TTL" width="460px" append-to-body>
            ...
            {{
              footer: () => (
                <div>
                  <el-button onClick={() => { ttlVisible.value = false; }}>取消</el-button>
