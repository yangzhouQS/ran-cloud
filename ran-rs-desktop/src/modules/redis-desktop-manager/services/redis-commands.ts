...
// Tauri 2 Plugin 命令前缀
// 通过 tauri::plugin::Builder 注册的命令会自动加上 plugin:<name>| 前缀
const PLUGIN_PREFIX = "plugin:redis-desktop|";

/** 带插件前缀的 invoke 封装 */
function pluginInvoke<T>(cmd: string, args?: Record<string, unknown>): Promise<T> {
  return invoke<T>(`${PLUGIN_PREFIX}${cmd}`, args);
}
...
export function redisStorageLoadConnections(): Promise<ConnectionConfig[]> {
  return pluginInvoke("redis_storage_load_connections");
}

/** 保存连接列表 */
export function redisStorageSaveConnections(connections: ConnectionConfig[]): Promise<void> {
  return pluginInvoke("redis_storage_save_connections", { connections });
}

/** 保存单个连接 */
export function redisStorageSaveConnection(connection: ConnectionConfig): Promise<void> {
  return pluginInvoke("redis_storage_save_connection", { connection });
}

/** 删除连接 */
export function redisStorageDeleteConnection(id: string): Promise<void> {
  return pluginInvoke("redis_storage_delete_connection", { id });
}

/** 加载设置 */
export function redisStorageLoadSettings(): Promise<AppSettings> {
  return pluginInvoke("redis_storage_load_settings");
}

/** 保存设置 */
export function redisStorageSaveSettings(settings: AppSettings): Promise<void> {
  return pluginInvoke("redis_storage_save_settings", { settings });
}

/** 加载 CLI 历史 */
export function redisStorageLoadCliHistory(connectionId: string): Promise<string[]> {
  return pluginInvoke("redis_storage_load_cli_history", { connectionId });
}

/** 保存 CLI 历史 */
export function redisStorageSaveCliHistory(connectionId: string, history: string[]): Promise<void> {
  return pluginInvoke("redis_storage_save_cli_history", { connectionId, history });
}
...
export function redisKeyScan(params: KeyScanParams): Promise<KeyScanResult[]> {
  return pluginInvoke("redis_key_scan", { params });
}

/** 启动流式 SCAN（后端通过 Tauri Event 推送进度） */
export function redisKeyScanStart(params: KeyScanParams): Promise<void> {
  return pluginInvoke("redis_key_scan_start", { params });
}

/** 取消流式 SCAN */
export function redisKeyScanCancel(connectionId: string): Promise<void> {
  return pluginInvoke("redis_key_scan_cancel", { connectionId });
}

/** 获取 Key 详情 */
export function redisKeyDetail(connectionId: string, db: number, key: string): Promise<KeyDetail> {
  return pluginInvoke("redis_key_detail", { connectionId, db, key });
}

/** 删除 Key */
export function redisKeyDelete(connectionId: string, db: number, keys: string[]): Promise<number> {
  return pluginInvoke("redis_key_delete", { connectionId, db, keys });
}

/** 重命名 Key */
export function redisKeyRename(connectionId: string, db: number, oldKey: string, newKey: string): Promise<void> {
  return pluginInvoke("redis_key_rename", { connectionId, db, oldKey, newKey });
}

/** 设置过期时间 */
export function redisKeyExpire(connectionId: string, db: number, key: string, seconds: number): Promise<void> {
  return pluginInvoke("redis_key_expire", { connectionId, db, key, seconds });
}
...
export function redisDataStreamAdd(
  ...
  return pluginInvoke("redis_data_stream_add", { connectionId, db, key, fields, id });
}

/** Stream: 删除条目 */
export function redisDataStreamDelete(params: DataDeleteParams): Promise<number> {
  return pluginInvoke("redis_data_stream_delete", { params });
}

/** Stream: 获取消费者组信息 */
export function redisDataStreamGroups(connectionId: string, db: number, key: string): Promise<StreamGroupInfo[]> {
  return pluginInvoke("redis_data_stream_groups", { connectionId, db, key });
}
...
export function redisCliExec(connectionId: string, db: number, command: string): Promise<CliExecResult> {
  return pluginInvoke("redis_cli_exec", { params: { connectionId, db, command } });
}

/** 命令自动补全 */
export function redisCliComplete(connectionId: string, input: string): Promise<string[]> {
  return pluginInvoke("redis_cli_complete", { connectionId, input });
}
...
export function redisToolSlowLog(connectionId: string, db: number, count: number): Promise<SlowLogEntry[]> {
  return pluginInvoke("redis_tool_slow_log", { connectionId, db, count });
}

/** 获取服务器状态 */
export function redisToolServerStatus(connectionId: string): Promise<ServerStatus> {
  return pluginInvoke("redis_tool_server_status", { connectionId });
}

/** 初始化命令日志（设置 AppHandle） */
export function redisToolCommandLogInit(): Promise<void> {
  return pluginInvoke("redis_tool_command_log_init");
}
...
export function redisToolCommandLogList(connectionId: string, limit: number): Promise<Array<{
  ...
  return pluginInvoke("redis_tool_command_log_list", { connectionId, limit });
}

/** 清除指定连接的命令日志 */
export function redisToolCommandLogClear(connectionId: string): Promise<void> {
  return pluginInvoke("redis_tool_command_log_clear", { connectionId });
}

/** 清除所有命令日志 */
export function redisToolCommandLogClearAll(): Promise<void> {
  return pluginInvoke("redis_tool_command_log_clear_all");
}
