...
export function redisStorageSaveConnection(connection: ConnectionConfig): Promise<void> {
  return invoke("redis_storage_save_connection", { connection });
}

/** 删除连接 */
export function redisStorageDeleteConnection(id: string): Promise<void> {
  return invoke("redis_storage_delete_connection", { id });
}
...
export function redisStorageLoadCliHistory(connectionId: string): Promise<string[]> {
  return invoke("redis_storage_load_cli_history", { connectionId });
}

/** 保存 CLI 历史 */
export function redisStorageSaveCliHistory(connectionId: string, history: string[]): Promise<void> {
  return invoke("redis_storage_save_cli_history", { connectionId, history });
}
...
export function redisKeyScanStart(params: KeyScanParams): Promise<void> {
  return invoke("redis_key_scan_start", { params });
}

/** 取消流式 SCAN */
export function redisKeyScanCancel(connectionId: string): Promise<void> {
  return invoke("redis_key_scan_cancel", { connectionId });
}
...
export function redisDataStreamAdd(
  connectionId: string, db: number, key: string,
  fields: [string, string][], id?: string,
): Promise<string> {
...
export function redisToolCommandLogList(connectionId: string, limit: number): Promise<Array<{
  ...
  return invoke("redis_tool_command_log_list", { connectionId, limit });
}
