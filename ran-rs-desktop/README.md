...
请在数据库链接完成后，展示当前链接可以访问的数据库和数据表，
交互需要在点击链接完成后，展示数据库列表，数据库可以展开，展示每个数据库下的数据表

## 2.SQL查询编辑器和查询结果布局调整

- SQL编辑器上边布局
- 下边为执行SQL结果的反馈展示（错误、执行的SQL）


## 3.前端项目模块拆分，每个模块支持单独打包与启动
- 前端拆分为layout模块，负责前端模块的整体布局功能
- 前端，F:\code999\demo\ran-cloud\ran-rs-desktop\src\modules 目录下的模块支持单独打包产出独立产物
- rsbuild entry 配置每个模块的入口文件，多入口配置
- 模块之间通过路由进行跳转和交互
- 重构后减少模块之间的代码耦合，架构更加清晰
